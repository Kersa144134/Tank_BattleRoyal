	...
	SubShader
	{
		...
		Pass
		{
				...
				, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				#else
				float staticSwitch385 = input.ase_texcoord7.xyz.y;
				#endif
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float staticSwitch385 = ( FinalWorldPosition102.y / ase_objectScale.y );
				#else
				float staticSwitch385 = input.ase_texcoord6.xyz.y;
				#endif
				float temp_output_109_0 = abs( ( (0.0 + (staticSwitch385 - 0.0) * (1.0 - 0.0) / (_MaskAppearLocalYRamap - 0.0)) + _MaskAppearLocalYAdd ) );
				#ifdef _MASKAPPEARINVERT_ON
				float staticSwitch118 = ( 1.0 - temp_output_109_0 );
		...
		Pass
		{
			...
			PackedVaryings VertexFunction(Attributes input  )
			{
				...
				float3 ase_worldTangent = TransformObjectToWorldDir(input.ase_tangent.xyz);
				output.ase_texcoord1.xyz = ase_worldTangent;
			...
			half4 frag(PackedVaryings input , bool ase_vface : SV_IsFrontFace) : SV_TARGET
			{
				...
				ase_worldViewDir = normalize(ase_worldViewDir);
				float3 switchResult27 = (((ase_vface>0)?(float3(0,0,1)):(float3(0,0,-1))));
				float3 ase_worldTangent = input.ase_texcoord1.xyz;
				...
				float staticSwitch385 = ( FinalWorldPosition102.y / ase_objectScale.y );
				#else
				float staticSwitch385 = input.ase_texcoord5.xyz.y;
				#endif
				float temp_output_109_0 = abs( ( (0.0 + (staticSwitch385 - 0.0) * (1.0 - 0.0) / (_MaskAppearLocalYRamap - 0.0)) + _MaskAppearLocalYAdd ) );
				#ifdef _MASKAPPEARINVERT_ON
				float staticSwitch118 = ( 1.0 - temp_output_109_0 );
				...
				float staticSwitch118 = temp_output_109_0;
				#endif
				float2 uv_MaskAppearNoise = input.ase_texcoord6.xy * _MaskAppearNoise_ST.xy + _MaskAppearNoise_ST.zw;
		...
		Pass
		{
				...
				, bool ase_vface : SV_IsFrontFace )
			{
				...
				float3 tanToWorld0 = float3( ase_worldTangent.x, ase_worldBitangent.x, input.clipPosV.xyz.x );
				float3 tanToWorld1 = float3( ase_worldTangent.y, ase_worldBitangent.y, input.clipPosV.xyz.y );
				float3 tanToWorld2 = float3( ase_worldTangent.z, ase_worldBitangent.z, input.clipPosV.xyz.z );
				float3 tanNormal8 = switchResult27;
				float3 worldNormal8 = float3(dot(tanToWorld0,tanNormal8), dot(tanToWorld1,tanNormal8), dot(tanToWorld2,tanNormal8));
				float dotResult1 = dot( ase_worldViewDir , worldNormal8 );
				float4 ase_screenPosNorm = ScreenPos / ScreenPos.w;
				...
				#endif
				float clampResult17 = clamp( max( pow( ( 1.0 - dotResult1 ) , _MaskFresnelExp ) , staticSwitch330 ) , 0.0 , 1.0 );
				float3 temp_output_57_0 = abs( input.clipPosV.xyz );
				float3 temp_output_60_0 = ( temp_output_57_0 * temp_output_57_0 );
				float4 transform106 = mul(GetObjectToWorldMatrix(),float4(0,0,0,1));
				float3 appendResult101 = (float3(transform106.x , transform106.y , transform106.z));
				#ifdef _LOCALNOISEPOSITION_ON
				...
				float3 FinalWorldPosition102 = staticSwitch103;
				float4 temp_cast_1 = (0.0).xxxx;
				float4 triplanar34 = TriplanarSampling34( _NoiseDistortion, ase_worldPos, input.clipPosV.xyz, 1.0, _NoiseDistortionTiling, 1.0, 0 );
				#ifdef _NOISEDISTORTIONENABLED_ON
				float4 staticSwitch42 = ( triplanar34 * _NoiseDistortionPower );
				#else
				float4 staticSwitch42 = temp_cast_1;
				...
				#endif
				float2 uv_MaskAppearNoise = input.ase_texcoord6.xy * _MaskAppearNoise_ST.xy + _MaskAppearNoise_ST.zw;
				float4 triplanar303 = TriplanarSampling303( _MaskAppearNoise, ase_worldPos, input.clipPosV.xyz, 1.0, _MaskAppearNoiseTriplanarTiling, 1.0, 0 );
				#ifdef _MASKAPPEARNOISETRIPLANAR_ON
				float staticSwitch304 = triplanar303.x;
				#else
				float staticSwitch304 = tex2D( _MaskAppearNoise, uv_MaskAppearNoise ).r;
...
Node;AmplifyShaderEditor.SamplerNode;147;3107.841,2575.91;Inherit;True;Property;_MaskAppearRamp;Mask Appear Ramp;33;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
...
Node;AmplifyShaderEditor.ColorNode;89;842.3301,-1064.289;Float;False;Property;_RampColorTint;Ramp Color Tint;8;0;Create;True;0;0;0;False;0;False;1,1,1,1;1,0.4893508,0.3014636,1;False;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;404;1914.879,-296.0403;Float;False;False;-1;2;UnityEditor.ShaderGraphUnlitGUI;0;1;New Amplify Shader;2992e84f91cbeb14eab234972e07ea9d;True;MotionVectors;0;10;MotionVectors;0;False;False;False;False;False;False;False;False;False;False;False;False;True;0;False;;False;True;0;False;;False;False;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;False;False;False;True;4;RenderPipeline=UniversalPipeline;RenderType=Opaque=RenderType;Queue=Geometry=Queue=0;UniversalMaterialType=Unlit;True;5;True;12;all;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;True;False;False;0;False;;False;False;False;False;False;False;False;False;False;False;False;False;True;1;LightMode=MotionVectors;False;False;0;;0;0;Standard;0;False;0
...
ASEEND*/
//CHKSM=0F596529961C7DC9486BBAFA0050E6C644EBA776